namespace HMRC.CTP.Specs.Scenarios.Request.Factory.Strategies
{
    ...
    internal class ChangeOfAddressRequest : IRequestBuilder
    {
        public RequestCreationEntity Build()
        {
            var faked = RequestFakerCommon.Create(CodedReasons.ChangeOfAddress).Generate();
            var address = ProposedAddressFaker.Create((DateTime)faked["voa_effectivedate"]);
            address["voa_duplicatesvalidatedondate"] = DateTime.UtcNow;
            ...
            faked["voa_swapbareferencenumber"] = true;
